...
func (sm2sig *sm2Signature) marshal() []byte {
	byteLen := 32
	ret := make([]byte, 1+2*byteLen)

	rBytes := sm2sig.R.Bytes()
	copy(ret[32-len(rBytes):byteLen], rBytes)
	sBytes := sm2sig.S.Bytes()
	copy(ret[2*byteLen-len(sBytes):], sBytes)

	ret[2*byteLen] = 1 // fake recId, more details, please refer EIP155

	return ret
}

func (sm2sig *sm2Signature) unmarshal(data []byte) bool {
	byteLen := 32
	if len(data) != 1+2*byteLen {
		return false
	}
	sm2sig.R = new(big.Int).SetBytes(data[0:byteLen])
	sm2sig.S = new(big.Int).SetBytes(data[byteLen:2*byteLen])

	if data[2*byteLen] != 1 { // uncompressed form
		return false
	}
...
func (pub *PublicKey) Verify(msg []byte, sign []byte) bool {
	var sm2Sign sm2Signature
	if succeed := sm2Sign.unmarshal(sign); !succeed {
		return false
	}
