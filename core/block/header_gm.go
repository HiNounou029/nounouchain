...
func (h *Header) Signer() (signer polo.Address, err error) {
	...
	sighash := h.SigningHash().Bytes()
	sig := h.body.Signature
	...
	public, _ := crypto.Ecrecover(sighash, sig)
	v := crypto.VerifySignature(public, sighash, sig) // for sm2, we have to verify the signature
	if v == true {
		signer = polo.Address(crypto.PubkeyToAddress(*pubkey))
		return
	}
	return polo.Address{}, errors.New("failed to verify block signature")
}
