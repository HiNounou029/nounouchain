...
type Server struct {
	...
	connectNodes    map[discover.NodeID]int
}
...
func New(opts *Options) *Server {
	...
	return &Server{
		...
		connectNodes:    make(map[discover.NodeID]int),
	}
}
...
func (s *Server) listenDiscV5() (err error) {
	...
	if s.opts.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			s.goes.Go(func() { nat.Map(s.opts.NAT, s.done, "udp", realaddr.Port, realaddr.Port, "polo discovery") })
		}
...
func (s *Server) dialLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			if _, ok := s.connectNodes[node.ID]; ok {
				continue
			}
			...
			go func() {
				if err := s.tryDial(node); err != nil {
					...
					if err == errors.New("DiscAlreadyConnected") {
						delete(s.connectNodes, node.ID)
					}
				}
			}()
