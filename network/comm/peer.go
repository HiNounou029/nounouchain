...
import (
	...
	"github.com/HiNounou029/nounouchain/network/rpc"
...
func (p *Peer) Head() (id polo.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) UpdateHead(id polo.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
