...
func checkTrieContents(t *testing.T, db Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(polo.BytesToBytes32(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, polo.BytesToBytes32(root)); err != nil {
...
func checkTrieConsistency(db Database, root polo.Bytes32) error {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(polo.Bytes32{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db := ethdb.NewMemDatabase()
		if req := NewTrieSync(polo.BytesToBytes32(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	sched := NewTrieSync(polo.BytesToBytes32(srcTrie.Root()), dstDb, nil)

	queue := append([]polo.Bytes32{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]polo.Bytes32{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[polo.Bytes32]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]polo.Bytes32{}, sched.Missing(0)...)
	requested := make(map[polo.Bytes32]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []polo.Bytes32{}
	queue := append([]polo.Bytes32{}, sched.Missing(1)...)
	for len(queue) > 0 {
