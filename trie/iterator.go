...
type nodeIteratorState struct {
	hash    polo.Bytes32 // Hash of the node being iterated (nil if not standalone)
	node    node         // Trie node being iterated
	parent  polo.Bytes32 // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() polo.Bytes32 {
	if len(it.stack) == 0 {
		return polo.Bytes32{}
	}
...
func (it *nodeIterator) Parent() polo.Bytes32 {
	if len(it.stack) == 0 {
		return polo.Bytes32{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = polo.BytesToBytes32(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor polo.Bytes32) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() polo.Bytes32 {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() polo.Bytes32 {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == polo.Bytes32{}
...
func (it *unionIterator) Hash() polo.Bytes32 {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() polo.Bytes32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == polo.Bytes32{}) {
			it.count += 1
