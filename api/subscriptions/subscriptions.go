...
func (s *Subscriptions) handleTransactionReader(w http.ResponseWriter, req *http.Request) (*transactionReader, error) {
	position, err := s.parsePosition(req.URL.Query().Get("pos"))
	if err != nil {
		return nil, err
	}

	txHash, err := parseHash(req.URL.Query().Get("txHash"))
	if err != nil {
		return nil, utils.BadRequest(errors.WithMessage(err, "transaction"))
	}

	transactionFilter := &TranscationFilter{
		TxHash: txHash,
	}
	return newTransactionReader(s.chain, position, transactionFilter), nil
}
...
func (s *Subscriptions) handleSubject(w http.ResponseWriter, req *http.Request) error {
	...
	case "transaction":
		if reader, err = s.handleTransactionReader(w, req); err != nil {
			return err
		}
	default:
	...
	if err != nil {
		log.Error("upgrade to websocket", "err", err)
		return nil
	}

	defer func() {
		if err := conn.Close(); err != nil {
			log.Error("close websocket", "err", err)
		}
	}()
...
func (s *Subscriptions) parsePosition(posStr string) (polo.Bytes32, error) {
	bestID := s.chain.BestBlock().Header().ID()
	if posStr == "" {
		return bestID, nil
	}
	pos, err := polo.ParseBytes32(posStr)
	if err != nil {
		return polo.Bytes32{}, utils.BadRequest(errors.WithMessage(err, "pos"))
	}
	if block.Number(bestID)-block.Number(pos) > s.backtraceLimit {
		return polo.Bytes32{}, utils.Forbidden(errors.New("pos: backtrace limit exceeded"))
	}
	return pos, nil
}

func parseTopic(t string) (*polo.Bytes32, error) {
	if t == "" {
		return nil, nil
	}
	topic, err := polo.ParseBytes32(t)
	if err != nil {
		return nil, err
	}
	return &topic, nil
...
func parseAddress(addr string) (*polo.Address, error) {
	if addr == "" {
		return nil, nil
	}
	address, err := polo.ParseAddress(addr)
	if err != nil {
		return nil, err
	}
	return &address, nil
...
func parseHash(hash string) (polo.Bytes32, error) {
	if hash == "" {
		return polo.Bytes32{}, nil
	}

	return polo.ParseBytes32(hash)
}
