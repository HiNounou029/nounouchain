...
var (
	headKey = polo.Blake2b([]byte("head"))
	tailKey = polo.Blake2b([]byte("tail"))
)
...
func New(addr polo.Address, state *state.State) *Authority {
	return &Authority{addr, state}
}

func (a *Authority) getEntry(nodeMaster polo.Address) *entry {
	var entry entry
	a.state.DecodeStorage(a.addr, polo.BytesToBytes32(nodeMaster[:]), func(raw []byte) error {
		if len(raw) == 0 {
			return nil
		}
...
func (a *Authority) setEntry(nodeMaster polo.Address, entry *entry) {
	a.state.EncodeStorage(a.addr, polo.BytesToBytes32(nodeMaster[:]), func() ([]byte, error) {
		if entry.IsEmpty() {
			return nil, nil
		}
...
func (a *Authority) getAddressPtr(key polo.Bytes32) (addr *polo.Address) {
...
func (a *Authority) setAddressPtr(key polo.Bytes32, addr *polo.Address) {
...
func (a *Authority) Get(nodeMaster polo.Address) (listed bool, endorsor polo.Address, identity polo.Bytes32, active bool) {
...
func (a *Authority) Add(nodeMaster polo.Address, endorsor polo.Address, identity polo.Bytes32) bool {
	entry := a.getEntry(nodeMaster)
	if !entry.IsEmpty() {
		return false
	}
...
func (a *Authority) Revoke(nodeMaster polo.Address) bool {
	entry := a.getEntry(nodeMaster)
	if !entry.IsLinked() {
		return false
	}
...
func (a *Authority) Update(nodeMaster polo.Address, active bool) bool {
	entry := a.getEntry(nodeMaster)
	if !entry.IsLinked() {
		return false
	}
