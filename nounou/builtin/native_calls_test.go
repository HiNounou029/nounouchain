...
func (c *ccase) To(to polo.Address) *ccase {
	c.to = to
	return c
}

func (c *ccase) Caller(caller polo.Address) *ccase {
	c.caller = caller
	return c
}
...
func (c *ccase) TxID(txID polo.Bytes32) *ccase {
	c.txID = txID
	return c
}
...
func TestParamsNative(t *testing.T) {
	executor := polo.BytesToAddress([]byte("e"))
	kv, _ := storage.NewMem()
	b0 := buildGenesis(kv, func(state *state.State) error {
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(polo.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		return nil
	})
	c, _ := chain.New(kv, b0)
...
func TestAuthorityNative(t *testing.T) {
	var (
		master1   = polo.BytesToAddress([]byte("master1"))
		endorsor1 = polo.BytesToAddress([]byte("endorsor1"))
		identity1 = polo.BytesToBytes32([]byte("identity1"))

		master2   = polo.BytesToAddress([]byte("master2"))
		endorsor2 = polo.BytesToAddress([]byte("endorsor2"))
		identity2 = polo.BytesToBytes32([]byte("identity2"))

		master3   = polo.BytesToAddress([]byte("master3"))
		endorsor3 = polo.BytesToAddress([]byte("endorsor3"))
		identity3 = polo.BytesToBytes32([]byte("identity3"))
		executor  = polo.BytesToAddress([]byte("e"))
	)
	...
	b0 := buildGenesis(kv, func(state *state.State) error {
		...
		state.SetBalance(polo.Address(endorsor1), polo.InitialProposerEndorsement)
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(polo.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		builtin.Params.Native(state).Set(polo.KeyProposerEndorsement, polo.InitialProposerEndorsement)
		return nil
	})
	c, _ := chain.New(kv, b0)
	st, _ := state.New(b0.Header().StateRoot(), kv)
	...
	candidateEvent := func(nodeMaster polo.Address, action string) *tx.Event {
		...
		return &tx.Event{
			...
			Topics:  []polo.Bytes32{ev.ID(), polo.BytesToBytes32(nodeMaster[:])},
			Data:    data,
		}
	}
	...
	test.Case("revoke", master2).
		Caller(polo.BytesToAddress([]byte("some one"))).
		Assert(t)
...
func TestPrototypeNative(t *testing.T) {
	var (
		acc1 = polo.BytesToAddress([]byte("acc1"))
		acc2 = polo.BytesToAddress([]byte("acc2"))

		master    = polo.BytesToAddress([]byte("master"))
		notmaster = polo.BytesToAddress([]byte("notmaster"))
		user      = polo.BytesToAddress([]byte("user"))
		notuser   = polo.BytesToAddress([]byte("notuser"))

		credit       = big.NewInt(1000)
		recoveryRate = big.NewInt(10)
		sponsor      = polo.BytesToAddress([]byte("sponsor"))
		notsponsor   = polo.BytesToAddress([]byte("notsponsor"))

		key      = polo.BytesToBytes32([]byte("account-key"))
		value    = polo.BytesToBytes32([]byte("account-value"))
		contract polo.Address
	)
	...
	st.SetStorage(polo.Address(acc1), key, value)
	st.SetBalance(polo.Address(acc1), big.NewInt(1))

	masterEvent := func(self, newMaster polo.Address) *tx.Event {
		ev, _ := builtin.Prototype.Events().EventByName("$Master")
		data, _ := ev.Encode(newMaster)
		return &tx.Event{
			Address: self,
			Topics:  []polo.Bytes32{ev.ID()},
			Data:    data,
		}
	}

	creditPlanEvent := func(self polo.Address, credit, recoveryRate *big.Int) *tx.Event {
	...
	userEvent := func(self, user polo.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []polo.Bytes32{ev.ID(), polo.BytesToBytes32(user.Bytes())},
			Data:    data,
		}
	}

	sponsorEvent := func(self, sponsor polo.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []polo.Bytes32{ev.ID(), polo.BytesToBytes32(sponsor.Bytes())},
			Data:    data,
		}
	}
	...
	test.Case("storageFor", acc1, key).
		ShouldOutput(value).
		Assert(t)
	test.Case("storageFor", acc1, polo.BytesToBytes32([]byte("some-key"))).
		ShouldOutput(polo.Bytes32{}).
		Assert(t)

	// should be hash of rlp raw
	test.Case("storageFor", builtin.Prototype.Address, polo.Blake2b(contract.Bytes(), []byte("credit-plan"))).
		ShouldOutput(st.GetStorage(builtin.Prototype.Address, polo.Blake2b(contract.Bytes(), []byte("credit-plan")))).
		Assert(t)
...
func TestPrototypeNativeWithLongerBlockNumber(t *testing.T) {
	...
	rt := runtime.New(seeker, st, &xenv.BlockContext{
		Number: polo.MaxBackTrackingBlockNumber + 1,
...
func TestExtensionNative(t *testing.T) {
	...
	test.Case("blake2b256", []byte("hello world")).
		ShouldOutput(polo.Blake2b([]byte("hello world"))).
		Assert(t)
	...
	test.Case("txID").
		TxID(polo.BytesToBytes32([]byte("txID"))).
		ShouldOutput(polo.BytesToBytes32([]byte("txID"))).
		Assert(t)
