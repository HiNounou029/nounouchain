...
func (db *LogDB) queryEvents(ctx context.Context, stmt string, args ...interface{}) ([]*Event, error) {
	...
	for rows.Next() {
		...
		event := &Event{
			BlockID:     polo.BytesToBytes32(blockID),
			Index:       index,
			BlockNumber: blockNumber,
			BlockTime:   blockTime,
			TxID:        polo.BytesToBytes32(txID),
			TxOrigin:    polo.BytesToAddress(txOrigin),
			Address:     polo.BytesToAddress(address),
			Data:        data,
		}
		for i, topic := range topics {
			if len(topic) > 0 {
				h := polo.BytesToBytes32(topic)
				event.Topics[i] = &h
			}
		}
...
func (db *LogDB) queryTransfers(ctx context.Context, stmt string, args ...interface{}) ([]*Transfer, error) {
	...
	for rows.Next() {
		...
		trans := &Transfer{
			...
			TxID:        polo.BytesToBytes32(txID),
			TxOrigin:    polo.BytesToAddress(txOrigin),
			Sender:      polo.BytesToAddress(sender),
			Recipient:   polo.BytesToAddress(recipient),
...
func topicValue(topic *polo.Bytes32) []byte {
	if topic == nil {
		return nil
	}
	return topic.Bytes()
...
func (bb *BlockBatch) Commit(abandonedBlocks ...polo.Bytes32) error {
	return bb.execInTx(func(tx *sql.Tx) error {
		...
		for _, transfer := range bb.transfers {
			...
			if transfer.Reverted == true {
				reverted = 1
			}

			if _, err := tx.Exec("INSERT OR REPLACE INTO transfer(blockID ,transferIndex, blockNumber ,blockTime ,txID ,txOrigin ,sender ,recipient ,amount ,reverted) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
...
func (bb *BlockBatch) ForTransaction(txID polo.Bytes32, txOrigin polo.Address) struct {
	Insert func(tx.Events, tx.Transfers, bool) *BlockBatch
} {
	return struct {
		Insert func(events tx.Events, transfers tx.Transfers, reverted bool) *BlockBatch
	}{
		func(events tx.Events, transfers tx.Transfers, reverted bool) *BlockBatch {
			for _, event := range events {
				bb.events = append(bb.events, newEvent(bb.header, uint32(len(bb.events)), txID, txOrigin, event))
			}
			for _, transfer := range transfers {
				bb.transfers = append(bb.transfers, newTransfer(bb.header, uint32(len(bb.transfers)), txID, txOrigin, transfer, reverted))
			}
			return bb
		},
	}
