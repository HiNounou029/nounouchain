...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	/*if len(input) == 68 {
		...
		if hexSig == transferSig {
			...
			toAddr := common.BytesToAddress(input[4:36])
			contractValue  := big.NewInt(0)
			contractValue.SetBytes(input[36:])
			log.Info("contract transfer", "addr", toAddr.String(), "value", contractValue)
			evm.StateDB.AddTransfer(&tx.Transfer{
				Sender:    polo.Address(caller.Address()),
