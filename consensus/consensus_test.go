...
func newTestConsensus(t *testing.T) *testConsensus {
	...
	gen := new(genesis.Builder).
		...
		State(func(state *state.State) error {
			...
			builtin.Params.Native(state).Set(polo.KeyExecutorAddress, new(big.Int).SetBytes(genesis.DevAccounts()[0].Address[:]))
			for _, acc := range genesis.DevAccounts() {
				state.SetBalance(acc.Address, bal)
				builtin.Authority.Native(state).Add(acc.Address, acc.Address, polo.Bytes32{})
			}
			return nil
		})
	...
	p := miner.New(c, stateCreator, proposer.Address, &proposer.Address)
...
func (tc *testConsensus) TestValidateBlockHeader() {
	...
	triggers["triggerErrFutureBlock"] = func() {
		build := tc.originalBuilder()
		blk := tc.sign(build.Timestamp(tc.time + polo.Conf.BlockInterval*2).Build())
		err := tc.consent(blk)
...
func (tc *testConsensus) TestValidateProposer() {
	...
	triggers["triggerErrSignerInvalid"] = func() {
		...
		expect := consensusError(
			fmt.Sprintf(
				...
				polo.Address(crypto.PubkeyToAddress(pk.PublicKey)),
			),
		)
		tc.assert.Equal(err, expect)
	}
	triggers["triggerErrTimestampUnscheduled"] = func() {
		...
		expect := consensusError(
			fmt.Sprintf(
				...
				blk.Header().Timestamp(),
				polo.Address(crypto.PubkeyToAddress(genesis.DevAccounts()[1].PrivateKey.PublicKey)),
			),
		)
		tc.assert.Equal(err, expect)
	}
	triggers["triggerTotalScoreInvalid"] = func() {
		build := tc.originalBuilder()
		fmt.Printf("original total score %d",tc.original.Header().TotalScore())
		...
		expect := "block total score invalid"
		tc.assert.Contains(err, expect)
	}
