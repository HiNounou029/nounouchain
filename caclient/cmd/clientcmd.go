...
const (
	polochainCAClientProfileMode = "POLOCHAIN_CA_CLIENT_PROFILE_MODE"
...
// for the PoloChain CA client and the configuration used by the PoloChain CA client
type ClientCmd struct {
	// name of the sub command
	name string
	// rootCmd is the base command for the Hyerledger PoloChain CA client
...
func NewCommand(name string) *ClientCmd {
	...
	//c.init()							// noted by syl 2018-08-29=======
	return c
}
...
//
func (c *ClientCmd) Init(flagsMap map[string]string) {
	c.init()

	// enroll
	if c.name == "enroll" {
		if flagsMap["u"] != "" {
			c.clientCfg.URL = flagsMap["u"]
			c.myViper.Set("url", flagsMap["u"])
		}
		if flagsMap["M"] != "" {
			c.clientCfg.MSPDir = flagsMap["M"]
			c.myViper.Set("mspdir", flagsMap["M"])
		}
	}

	// register
	if c.name == "register" {
		c.clientCfg.ID.Name = flagsMap["id.name"]
		c.clientCfg.ID.Type = flagsMap["id.type"]
		c.clientCfg.ID.Affiliation = flagsMap["id.affiliation"]
		attrs := flagsMap["id.attrs"]
		if strings.Index(attrs, "'") != -1 {
			attrs = attrs[1:(len(attrs)- 1)]
		}
		c.cfgAttrs = strings.Split(attrs, ",")

		c.myViper.Set("id.name", flagsMap["id.name"])
		c.myViper.Set("id.type", flagsMap["id.type"])
		c.myViper.Set("id.affiliation", flagsMap["id.affiliation"])
	}
}
...
func (c *ClientCmd) init() {
	...
	//	Use:   "version",
	//	Short: "Prints PoloChain CA Client version",
...
// checkAndEnableProfiling checks for the POLOCHAIN_CA_CLIENT_PROFILE_MODE
...
func (c *ClientCmd) checkAndEnableProfiling() error {
	c.profileMode = strings.ToLower(os.Getenv(polochainCAClientProfileMode))
	if c.profileMode != "" {
		wd, err := os.Getwd()
		if err != nil {
			wd = os.Getenv("HOME")
		...
		default:
			msg := fmt.Sprintf("Invalid value for the %s environment variable; found '%s', expecting 'cpu' or 'heap'",
				polochainCAClientProfileMode, c.profileMode)
