...
const (
	longName     = "Bluestone PoloChain Certificate Authority Client"
	shortName    = "polochain-ca client"
	cmdName      = "polochain-ca-server"
	envVarPrefix = "POLOCHAIN_CA_CLIENT"
	homeEnvVar   = "POLOCHAIN_CA_CLIENT_HOME"
)

const (
	defaultCfgTemplate = `
#############################################################################
#   This is a configuration file for the polochain-ca-server command.
#
...
#         To set the polochain-ca server url
#      b) --tls.client.certfile certfile.pem
#         To set the client certificate for TLS
#   2) environment variable
#      Examples:
#      a) POLOCHAIN_CA_CLIENT_URL=https://localhost:7054
#         To set the polochain-ca server url
#      b) POLOCHAIN_CA_CLIENT_TLS_CLIENT_CERTFILE=certfile.pem
#         To set the client certificate for TLS
...
# URL of the PoloChain-ca-server (default: http://localhost:7054)
url: <<<URL>>>
...
#############################################################################
#  Registration section used to register a new identity with polochain-ca server
#
...
#############################################################################
#  Enrollment section used to enroll an identity with polochain-ca server
#
...
#############################################################################
# Name of the CA to connect to within the polochain-ca server
#############################################################################
caname:
...
// ConfigInit initializes the configuration for the polochain-ca-server command
...
func (c *ClientCmd) createDefaultConfigFile() error {
	...
	var cfg string
	polochainCAServerURL := c.myViper.GetString("url")
	if polochainCAServerURL == "" {
		polochainCAServerURL = util.GetServerURL()
	} else {
		URL, err := url.Parse(polochainCAServerURL)
		if err != nil {
			return errors.Wrapf(err, "Failed to parse URL '%s'", polochainCAServerURL)
		}
		polochainCAServerURL = fmt.Sprintf("%s://%s", URL.Scheme, URL.Host)
	}

	myhost := c.myViper.GetString("myhost")

	// Do string subtitution to get the default config
	cfg = strings.Replace(defaultCfgTemplate, "<<<URL>>>", polochainCAServerURL, 1)
