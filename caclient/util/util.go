...
func CreateClientHome() (string, error) {
	...
	home := filepath.Dir(GetDefaultConfigFile("polochain-ca-server"))
...
func GetDefaultConfigFile(cmdName string) string {
	if cmdName == "polochain-ca-server" {
		var fname = fmt.Sprintf("%s-config.yaml", cmdName)
		// First check home env variables
		home := "."
		envs := []string{"POLOCHAIN_CA_SERVER_HOME", "POLOCHAIN_CA_HOME", "CA_CFG_PATH"}
	...
	// First check home env variables
	var home string
	envs := []string{"POLOCHAIN_CA_CLIENT_HOME", "POLOCHAIN_CA_HOME", "CA_CFG_PATH"}
	for _, env := range envs {
		envVal := os.Getenv(env)
		if envVal != "" {
			home = envVal
	...
	return path.Join(dir, "polochain-ca-server", fname)
	//return path.Join(os.Getenv("HOME"), "polochain-ca-server", fname)
}
...
func GetUser(v *viper.Viper) (string, string, error) {
	var polochainCAServerURL string
	polochainCAServerURL = v.GetString("url")
	...
	user := URL.User
	if user == nil {
		return "", "", errors.New("No username and password provided as part of the PoloChain CA server URL")
	}
