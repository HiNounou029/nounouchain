...
import (
	...
	idemix "github.com/HiNounou029/nounouchain/caclient/lib/server/idemix"
...
func (ca *CA) getCACert() (cert []byte, err error) {
	if ca.Config.Intermediate.ParentServer.URL != "" {
		// This is an intermediate CA, so call the parent polochain-ca-server
		// to get the cert
		...
		if ca.Config.CSR.CN == "" {
			ca.Config.CSR.CN = "polochain-ca-server"
		}
		csr := &ca.Config.CSR
		if csr.CA == nil {
...
func (ca *CA) initDB() error {
	...
	if db.Type == "" || db.Type == defaultDatabaseType {
		...
		if db.Datasource == "" {
			db.Datasource = "polochain-ca-server.db"
		}
