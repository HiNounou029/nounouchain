...
func getIDs(ctx *serverRequestContextImpl, caller spi.User, caname string) error {
	...
	numberOfIdentities := os.Getenv("POLOCHAIN_CA_SERVER_MAX_IDS_PER_CHUNK")
	...
	if numberOfIdentities == "" {
		...
		if err != nil {
			return newHTTPErr(500, ErrGettingUser, "Incorrect format specified for environment variable 'POLOCHAIN_CA_SERVER_MAX_IDS_PER_CHUNK', an integer value is required: %s", err)
		}
	}
