...
// PoloChainCADB is the interface with functions implemented by sqlx.DB
// object that are used by PoloChain CA server
type PoloChainCADB interface {
	IsInitialized() bool
	Select(dest interface{}, query string, args ...interface{}) error
	NamedExec(query string, arg interface{}) (sql.Result, error)
	Rebind(query string) string
	MustBegin() *sqlx.Tx
	// BeginTx has same behavior as MustBegin except it returns PoloChainCATx
	// instead of *sqlx.Tx
	BeginTx() PoloChainCATx
}

// PoloChainCATx is the interface with functions implemented by sqlx.Tx
// object that are used by PoloChain CA server
type PoloChainCATx interface {
	Select(dest interface{}, query string, args ...interface{}) error
	Rebind(query string) string
...
// DB is an adapter for sqlx.DB and implements PoloChainCADB interface
type DB struct {
	*sqlx.DB
...
// BeginTx implements BeginTx method of PoloChainCADB interface
func (db *DB) BeginTx() PoloChainCATx {
	return db.MustBegin()
}
