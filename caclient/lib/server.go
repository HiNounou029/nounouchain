...
import (
	...
	"github.com/HiNounou029/nounouchain/caclient/lib/metadata"
	stls "github.com/HiNounou029/nounouchain/caclient/lib/tls"
...
const (
	...
	polochainCAServerProfilePort = "POLOCHAIN_CA_SERVER_PROFILE_PORT"
	allRoles                  = "peer,orderer,client,user"
	apiPathPrefix             = "/api/v1/"
)

// Server is the polochain-ca server
type Server struct {
...
// Init initializes a polochain-ca server
...
func (s *Server) createDefaultCAConfigs(cacount int) error {
	...
	for i := 1; i <= cacount; i++ {
		...
		cfgFileName := filepath.Join(cahome, "polochain-ca-config.yaml")

		caName := fmt.Sprintf("ca%d", i)
		cfg := strings.Replace(defaultCACfgTemplate, "<<<CANAME>>>", caName, 1)

		cn := fmt.Sprintf("polochain-ca-server-ca%d", i)
...
// checkAndEnableProfiling checks for POLOCHAIN_CA_SERVER_PROFILE_PORT env variable
...
func (s *Server) checkAndEnableProfiling() error {
	...
	pport := os.Getenv(polochainCAServerProfilePort)
	if pport != "" {
...
func (s *Server) loadDNFromCertFile(certFile string) (*DN, error) {
	...
	distinguishedName := &DN{
		issuer:  RDNSequence2String(cert.Issuer.ToRDNSequence()),
		subject: RDNSequence2String(cert.Subject.ToRDNSequence()),
	}
...
func (s *Server) autoGenerateTLSCertificateKey() error {
	...
	// Can't use the same CN as the signing certificate CN (default: polochain-ca-server) otherwise no AKI is generated
