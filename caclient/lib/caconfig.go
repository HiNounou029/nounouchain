...
import (
	...
	"github.com/HiNounou029/nounouchain/caclient/lib/dbutil"
	"github.com/HiNounou029/nounouchain/caclient/lib/ldap"
	"github.com/HiNounou029/nounouchain/caclient/lib/server/idemix"
	"github.com/HiNounou029/nounouchain/caclient/lib/tls"
	"github.com/HiNounou029/nounouchain/caclient/util"
	"github.com/HiNounou029/nounouchain/caclient/bccsp/factory"
)
...
# A single polochain-ca-server can service multiple CAs.  The server's configuration
...
# CA name (see "ca.name" below).  Each CA name in the same polochain-ca-server
...
#  may not be used if you want to run the polochain-ca-server in a cluster.
#  To run the polochain-ca-server in a cluster, you must choose "postgres"
#  or "mysql".
#############################################################################
db:
...
// The tags are recognized by the RegisterFlags function in polochain-ca/util/flag.go
// and are as follows:
...
// CAInfo is the CA information on a polochain-ca-server
type CAInfo struct {
...
type CAConfigDB struct {
	...
	Datasource string `def:"polochain-ca-server.db" help:"Data source which is database specific"`
...
type ParentServer struct {
	URL    string `opt:"u" help:"URL of the parent polochain-ca-server (e.g. http://<username>:<password>@<address>:<port)" mask:"url"`
	CAName string `help:"Name of the CA to connect to on polochain-ca-server"`
}
